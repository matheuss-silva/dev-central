document.addEventListener("DOMContentLoaded", () => {
    ...
    let lastReceivedStatus = null;  // ✅ Evita logs repetitivos de status
    ...
    eventSocket.onmessage = (event) => {
        ...
        // ✅ Evita logs repetitivos para o mesmo status
        if (lastReceivedStatus === data.status) {
            return;
        }
        
        lastReceivedStatus = data.status; // Atualiza o cache local

        console.log(`🔄 Atualização do evento recebida: ${data.status}`);
        updateEvent(data);
    ...
    eventSocket.onclose = () => {
        console.warn("⚠️ WebSocket de evento desconectado. Tentando reconectar...");
        setTimeout(() => {
